  ...
  return (
    <li className={`px-2 py-1 flex items-start justify-between ${isExpandable ? 'border-1 border-text-gray' : ''}`}>
      <div className='flex flex-col gap-1'>
        ...
        <p className={`  text-lg font-extrabold  ${(request.status === ApiDonationScheduleStatus.PENDING || request.status === ApiDonationScheduleStatus.APPROVED) ? "text-yellow"
          : request.status === ApiDonationScheduleStatus.COMPLETED ? "text-green" : "text-red"
        }`}>{ request.status}</p>
      </div>
      {isExpandable && (
        <button
